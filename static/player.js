...
function vol(e){
    e.stopPropagation();
    if(draggingVolume || this == playing.volume.slider){
        with(playing.volume.slider){
            var sliderWidth = parseInt(style.width);
            var rawWidth = e.clientX - recurseoffset(inner)[1];
            var clampedWidth = Math.max(0, Math.min(rawWidth, sliderWidth));
            inner.style.width = clampedWidth + 'px';
            globalVolume = clampedWidth / sliderWidth;
...
function loadplayer(){
    ...
    addListener(document, 'mouseup', function(){
        ...
        if(draggingVolume){
            ...
            removeListener(document, 'mousemove', vol);
        }
    });
